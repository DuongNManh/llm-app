...
TEST_CASES = [
    ...
    {"id": 11, "question": "Điện thoại SM-A125F/DS có hỗ trợ Dolby Atmos không?",
...
COLLECTION_1 = "chunk1000va200"
COLLECTION_2 = "chunk512va128"
...
def create_vectorstore(
    ...
    collection_name : str = COLLECTION_2,
...
def load_vectorstore(
    per_dir: str = CHROMA_DIR,
    collection_name : str = COLLECTION_2) -> Chroma:
...
def save_report(all_results: dict, path: str):
    ...
    for variant_name, result in all_results.items():
        ...
        for r in result["details"]:
            ...
            lines.append(f"     Q: {r['answer_preview']}")
            lines.append("")
...
def run_evaluation(name: str, retriever, chain, test_cases: list) -> dict:
    ...
    for tc in test_cases:
        ...
        expected_pages = tc.get("expected_source_page", [])
...
def build_hybrid_retriever(vectorstore : Chroma, documents: list, k: int = HYBRID_K) -> EnsembleRetriever:
...
def indexing_pipeline(pdf_path: str = PDF_PATH, chroma_dir: str = CHROMA_DIR, collection_name: str = COLLECTION_2, use_hybrid: bool = True):
...
def test_chunk_num():
    ...
    docs = load_pdf()
    chunks1 = split_document_v1(docs, chunk_size=512, chunk_overlap=128)
    chunks2 = split_document_v1(docs, chunk_size=1000, chunk_overlap=200)
...
if __name__ == "__main__":
    if not settings.GEMINI_API_KEY:
        ...
        # indexing_pipeline(pdf_path=PDF_PATH, chroma_dir=CHROMA_DIR, collection_name=COLLECTION_2, use_hybrid=True)
